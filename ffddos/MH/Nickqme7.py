...
def install_requirements():
    ...
    print(f"{CYAN}กำลังติดตั้งโมดูลจาก requirements.txt...{RESET}")
    try:
        # ติดตั้งโมดูลจาก requirements.txt
        subprocess.run(
            [sys.executable, "-m", "pip", "install", "-r", "requirements.txt"],
                    )
                    
        print(f"{GREEN}ติดตั้งโมดูลสำเร็จ หรือโมดูลทั้งหมดมีอยู่แล้ว!{RESET}")
    except subprocess.CalledProcessError as e:
        print(f"{RED}เกิดข้อผิดพลาดในการติดตั้งโมดูล: {e.stderr.decode().strip()}{RESET}")
        sys.exit(1)
...
def run_command(ip_address):
    ...
    command = f"python3 /start.py UDP {ip_address} 53 999"
...
def get_ip_or_undo(previous_ip_address):
    ...
    while True:
        user_input = input(f"{CYAN}กรุณาป้อน IP Address ใหม่ หรือพิมพ์ 'b'/'ิ' เพื่อย้อนกลับ: {RESET}").strip()
...
if ip_address:
    ...
    try:
        while True:
            # รอคำสั่งจากผู้ใช้
            user_input = input(f"{CYAN}พิมพ์ 'ผ/z' เพื่อหยุด, 'ป/x' เพื่อรันต่อ, 'แ/c' เพื่อเปลี่ยน IP หรือ 'exit' เพื่อออก: {RESET}").strip().lower()
